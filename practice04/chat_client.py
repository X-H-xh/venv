...
def anythingllm_query(query, api_key, workspace_slug):
    ...
    try:
        ...
        if result.returncode != 0:
            return f"Error: curl command failed with code {result.returncode}. Stderr: {result.stderr}"
        
        if not result.stdout:
            return "Error: No response from AnythingLLM server"
        ...
        if 'textResponse' in response_data:
            ...
            return f"Response from AnythingLLM: {json.dumps(response_data, ensure_ascii=False)}"
    
    except FileNotFoundError:
        return "Error: curl command not found. Please install curl or use Windows with curl available."
    except Exception as e:
        return f"Error querying AnythingLLM: {str(e)}"
...
def should_query_anythingllm(user_input):
    user_input_lower = user_input.lower().strip()
    keywords = ['文档仓库', '文件仓库', '仓库', 'anythingllm', '知识库', '文档库', '查找文档', '搜索文档', '查文档']
    for keyword in keywords:
        if keyword in user_input_lower:
            return True
    return False
...
def execute_tool_call(tool_call, api_key, workspace_slug):
    function_name = tool_call['function']['name']
    arguments = json.loads(tool_call['function']['arguments'])
    
    if function_name == 'anythingllm_query':
        query = arguments.get('query', '')
        return anythingllm_query(query, api_key, workspace_slug)
...
def main():
    ...
    system_prompt = """你是一个友好的AI助手，具备以下工具功能：

1. anythingllm_query(query) - 用于查询AnythingLLM文档仓库，当用户提到"文档仓库"、"文件仓库"、"仓库"、"知识库"等相关词汇时，使用此工具查询AnythingLLM中的文档。
...
请根据用户的问题，决定是否需要调用工具。"""
    ...
    while True:
        ...
        if should_query_anythingllm(user_input):
            if not all([anythingllm_api_key, anythingllm_workspace]):
                print(f"\n>>> Error: Please configure ANYTHINGLLM_API_KEY and ANYTHINGLLM_WORKSPACE_SLUG in .env file")
                continue
            
            print(f"\n>>> 检测到文档仓库查询请求")
            print(f">>> 正在查询AnythingLLM...")
